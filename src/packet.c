...
static bool ssh_packet_need_rekey(ssh_session session, const uint32_t payloadsize)
{
...
int ssh_packet_socket_callback(const void *data, size_t receivedlen, void *user)
{
    ...
    switch(session->packet_state) {
        ...
        case PACKET_STATE_SIZEREAD:
            ...
            if (ssh_packet_need_rekey(session, 0)) {
...
static int packet_send2(ssh_session session)
{
    ...
    SSH_LOG(SSH_LOG_PACKET,
            "packet: wrote [type=%d,len=%d,padding_size=%hhd,comp=%d,payload=%d]",
            type, finallen, padding_size, compsize, payloadsize);
                     ...
                     struct ssh_cipher_struct *cipher)
{
    ...
    SSH_LOG(SSH_LOG_PROTOCOL, "Set rekey after %" PRId64 " blocks",
            cipher->max_blocks);
}
                       ...
                       enum ssh_crypto_direction_e direction)
{
    ...
    SSH_LOG(SSH_LOG_TRACE, "called, direction =%s%s",
    ...
    if (session->current_crypto) {
        if (session->current_crypto->used & direction) {
            SSH_LOG(SSH_LOG_WARNING, "This direction is not used anymore");
        }
    ...
    if (session->next_crypto->used == SSH_DIRECTION_BOTH) {
        ...
        memcpy(session->next_crypto->session_id,
               session->current_crypto->session_id, digest_len);
    ...
    if (session->opts.rekey_time != 0) {
        ...
        SSH_LOG(SSH_LOG_PROTOCOL, "Set rekey after %" PRId32 " seconds",
